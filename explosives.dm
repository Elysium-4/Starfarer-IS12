...
/obj/item/breacher
	...
	desc = "The big brother of your average explosives. Used to rip chunks of strong walls. Stay back for this one!"
	gender = PLURAL
	icon = 'icons/obj/assemblies.dmi'
	icon_state = "breach-charge0"
	item_state = "plasticx"
	item_flags = ITEM_FLAG_NO_BLUDGEON
	w_class = ITEM_SIZE_SMALL
	origin_tech = list(TECH_ILLEGAL = 2)
...
/obj/item/breacher/New()
	wires = new(src)
	image_overlay = image('icons/obj/assemblies.dmi', "breach-charge2")
	..()

/obj/item/breacher/Destroy()
	qdel(wires)
	wires = null
	return ..()

/obj/item/breacher/attackby(var/obj/item/I, var/mob/user)
	if(isScrewdriver(I))
		open_panel = !open_panel
		to_chat(user, "<span class='notice'>You [open_panel ? "open" : "close"] the wire panel.</span>")
	else if(isWirecutter(I) || isMultitool(I) || istype(I, /obj/item/device/assembly/signaler ))
...
/obj/item/breacher/attack_self(mob/user as mob)
	var/newtime = input(usr, "Please set the timer.", "Timer", 10) as num
	if(user.get_active_hand() == src)
		newtime = Clamp(newtime, 10, 60000)
		timer = newtime
...
/obj/item/breacher/afterattack(atom/movable/target, mob/user, flag)
	if (!flag)
		return
	if(!istype(target, /obj/machinery/door) && !iswall(target))
		return
...
/obj/item/breacher/proc/explode(var/location)
	if(!target)
		target = get_atom_on_turf(src)
	if(!target)
		target = src
	if(location)
		explosion(location, -1, -1, 4, 6)
...
/obj/item/breacher/attack(mob/M as mob, mob/user as mob, def_zone)
	return